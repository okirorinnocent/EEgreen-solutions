...
st.markdown(f"""
    ...
    h1, h2, h3 {{ color: #053e08 !important; font-weight: 700 !important; }}
    [data-testid="stMetricValue"] {{ font-size: 1.8rem !important; color: #2e7d32 !important; }}
    .stButton>button {{ border-radius: 4px; height: 3em; width: 100%; }}
    </style>
...
if choice == "🛒 Record Sales":
            ...
            if row['Stock Remaining'] > 0:
                with st.container():
                    ...
                    with col1:
                        ...
                        st.write(
                            f"Price: **UGX {row['Price per Unit']:,.0f}**")
                    with col2:
                        st.metric("Units Available", int(
                            row['Stock Remaining']))
                    with col3:
                        qty_sold = st.number_input("Amount Sold", min_value=0, max_value=int(
                            row['Stock Remaining']), key=f"sell_{index}")
    ...
    if not df.empty:
        ...
        if not low.empty:
            st.error(
                f"⚠️ Warning: {', '.join(low['Item Name'].tolist())} are running low!")
        ...
        for index, row in df.iterrows():
            with st.expander(f"⚙️ Manage {row['Item Name']}"):
                ...
                with c1:
                    st.write(
                        f"**Current Total Stock:** {row['Total Stock Input']}")
                    st.write(
                        f"**Currently on Shelf:** {row['Stock Remaining']}")
                    add_stock = st.number_input(
                        "Add New Shipment Qty", min_value=0, key=f"add_{index}")
                    new_p = st.number_input("Update Price", value=float(
                        row['Price per Unit']), key=f"p_{index}")
                    if st.button("Update Stock 🔄", key=f"up_{index}"):
                        new_input = row['Total Stock Input'] + add_stock
                        new_rem = row['Stock Remaining'] + add_stock
                        update_db(row['Item Name'], new_input,
                                  new_rem, new_p, user_name)
    ...
    if not df.empty:
        st.metric("Total Revenue Collected",
                  f"UGX {df['Total Sales Value'].sum():,.0f}")
        display_df = df[["Item Name", "Stock Sold Out",
                         "Stock Remaining", "Total Sales Value"]]
        display_df.columns = ["Product Name",
                              "Units Sold", "In Stock", "Revenue (UGX)"]
        st.table(display_df)
